...
const login = async (req, res, next) => {
    try {
        ...
        if (!userCheck) return res.status(404).json("No hay ningún usuario con ese nombre.");

        const passCheck = bcrypt.compareSync(req.body.password, userCheck.password);
        if (passCheck) {
...
const changePassword = async (req, res, next) => {
    try {
        ...
        return res.status(201).json("Contraseña cambiada con éxito");
    } catch (error) {
        return res.status(500).json(`Error (changePassword): ${error}`);
    }
};
